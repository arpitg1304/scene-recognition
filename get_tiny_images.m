...
for i = 1:R
    %resizing the image
    im = imread(image_paths{i,1});
    im_re = imresize(im, [16 16]);
    temp_image_feat = reshape(im_re, [256, 1]);
    
    %normalization of the image
    temp_image_feats = double(temp_image_feat);
    temp_image_feats = temp_image_feats ./ sum(temp_image_feats);
    temp_image_feats = temp_image_feats - mean(temp_image_feats);
    
    image_feats = [image_feats, temp_image_feats];
